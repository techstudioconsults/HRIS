...
export const BatchProgress: React.FC<BatchProgressProperties> = ({
  ...
  return (
    <div
      className={cn("flex items-center justify-between gap-4", size === "md" && "text-sm", className)}
      role="group"
      aria-label="Batch operation progress"
    >
      <div
        className={cn("text-muted-foreground flex items-center justify-between", size === "md" ? "text-sm" : "text-xs")}
        aria-live={ariaLive}
        aria-atomic="true"
      >
        <span className={cn("font-medium", hidePercentage && "sr-only")} aria-label={percentageLabel}>
          {percentageLabel}
        </span>
        <span className="max-w-[60%] truncate text-right font-medium" title={status}>
          {status}
        </span>
      </div>
      <Progress
