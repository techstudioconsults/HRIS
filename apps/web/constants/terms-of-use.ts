export const TERMS_OF_USE_CONTENT = {
  lastUpdated: '04/03/2026',
  intro: {
    title: 'Terms Of Use',
    description:
      'By accessing or using Techstudio HR, you agree to these Terms.',
  },
  sections: [
    {
      id: 1,
      title: 'Use of Service',
      content:
        'You agree to use the platform only for lawful purposes and in compliance with applicable regulations.',
    },
    {
      id: 2,
      title: 'Account Responsibility',
      content: 'You are responsible for:',
      items: [
        'Maintaining login confidentiality',
        'All activities under your account',
        'Providing accurate information',
      ],
    },
    {
      id: 3,
      title: 'Subscription & Billing',
      items: [
        'Plans renew automatically unless cancelled',
        'Pricing may change with notice',
        'Taxes may apply depending on region',
      ],
    },
    {
      id: 4,
      title: 'Data Responsibility',
      content:
        'Organizations remain the data controllers of employee information uploaded to the platform. Techstudio acts as a data processor.',
    },
    {
      id: 5,
      title: 'Acceptable Use',
      content: 'You may not:',
      items: [
        'Reverse engineer the platform',
        'Attempt unauthorized access',
        'Upload malicious content',
        'Use the service for illegal activities',
      ],
    },
    {
      id: 6,
      title: 'Service Availability',
      content:
        'We strive for high uptime but do not guarantee uninterrupted service. Maintenance or outages may occur.',
    },
    {
      id: 7,
      title: 'Limitation of Liability',
      content: 'Techstudio HR is not liable for:',
      items: [
        'Indirect damages',
        'Loss of profits',
        'Data loss beyond our control',
      ],
      note: 'Use of the platform is at your own risk.',
    },
    {
      id: 8,
      title: 'Termination',
      content: 'We reserve the right to suspend accounts that:',
      items: [
        'Violate these terms',
        'Pose security risks',
        'Abuse the platform',
      ],
    },
    {
      id: 9,
      title: 'Changes to Terms',
      content:
        'We may update these Terms at any time. Continued use implies acceptance.',
    },
    {
      id: 10,
      title: 'Governing Law',
      content:
        'These Terms are governed by the laws of [Insert Country — e.g., Nigeria]',
    },
  ],
};
