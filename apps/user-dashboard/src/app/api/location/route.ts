  ...
  try {
    switch (type) {
      ...
      case 'states': {
        ...
        if (!country) {
          return NextResponse.json(
            { error: 'Country parameter is required for states' },
            { status: 400 }
          );
        }
      ...
      case 'cities': {
        ...
        if (!country) {
          return NextResponse.json(
            { error: 'Country parameter is required for cities' },
            { status: 400 }
          );
        }
      ...
      default: {
        return NextResponse.json(
          {
            error:
              "Invalid type parameter. Use 'countries', 'states', or 'cities'",
          },
          { status: 400 }
        );
      }
...
async function getCountries(): Promise<NextResponse> {
  const response = await fetch(
    `${REST_COUNTRIES_API}/all?fields=name,flags,cca2`,
    {
...
async function getCities(
  ...
  if (state && state.trim()) {
    const response = await fetch(
      `${COUNTRIES_NOW_API}/countries/state/cities`,
      {
        method: 'POST',
        headers: {
