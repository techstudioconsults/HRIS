...
export const useResourceService = () => {
  const { useServiceQuery, useServiceMutation } =
    createServiceHooks<ResourceService>(dependencies.RESOURCE_SERVICE);
  ...
  const useGetAllFolders = (
    ...
    useServiceQuery(
      queryKeys.folder.list(filters),
      (service) => service.getAllFolders(filters),
      options
    );

  const useGetFolderById = (id: string, options?: any) =>
    useServiceQuery(
      queryKeys.folder.details(id),
      (service) => service.getFolderById(id),
      options
    );
  ...
  const useGetAllFiles = (filters: FileQueryParameters = {}, options?: any) =>
    useServiceQuery(
      queryKeys.file.list(filters),
      (service) => service.getAllFiles(filters),
      options
    );

  const useGetFilesByFolderId = (
    folderId: string,
  ...
  // Mutations - Folders
  const useDownloadFolder = () =>
    useServiceMutation((service, id: string) => service.downloadFolder(id));

  const useCreateFolder = () =>
    useServiceMutation(
      (service, data: { name: string; file: File[] }) =>
        service.createFolder(data.name, data.file),
      {
        invalidateQueries: () => [
          ['folder', 'list'],
          ['file', 'list'],
  ...
  const useUpdateFolder = () =>
    useServiceMutation(
      (
        service,
        { id, data }: { id: string; data: { name?: string; file?: File[] } }
      ) => service.updateFolder(id, data),
      {
  ...
  const useDownloadFile = () =>
    useServiceMutation((service, id: string) => service.downloadFile(id));

  const useAddFilesToFolder = () =>
    useServiceMutation(
      (service, { folderId, files }: { folderId?: string; files: File[] }) =>
        service.addFilesToFolder(folderId, files),
      {
  ...
  const useRemoveFileById = () =>
    useServiceMutation(
      (service, fileId: string) => service.removeFileByID(fileId),
      {
        invalidateQueries: () => [
          ['file', 'list'],
          ['folder', 'list'],
