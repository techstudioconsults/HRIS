...
export async function createRole(
  ...
  const response = await http.post<{ data: RoleApiResponse; success: boolean }>(
    `/roles`,
    roleData
  );
  if (response?.status === 201) {
    const { id, name, teamId, permissions, createdAt, updatedAt } =
      response.data.data;
    return {
      id,
      name,
...
export async function deleteRole(
  ...
  const response = await http.delete<{ success: boolean; data: string }>(
    `/roles/${roleId}`
  );
