...
export const useEmployeeRowActions = () => {
  ...
  const { entity: activeEmployee, set: setActiveEmployee } =
    useActiveTarget<Employee>();
  ...
  const handleDeleteEmployee = useCallback(async () => {
    ...
    try {
      ...
      toast.success(
        `Employee ${employeeToDelete.firstName} ${employeeToDelete.lastName} deleted successfully!`
      );
      resetModalState();
    } catch (error) {
      const message =
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener('employee:request-delete', onDeleteRequest);
  }, [activeEmployee]);
