...
export const AddNewEmployees = ({
  ...
  const [employees, setEmployees] = useState<FormEmployee[]>([
    { employeeId: '', roleId: '', customPermissions: [] },
  ]);
  const [openCustomPermissions, setOpenCustomPermissions] = useState<boolean[]>(
    [false]
  );
  ...
  const removeEmployee = (index: number) => {
    if (employees.length > 1) {
      ...
      setOpenCustomPermissions((previous) =>
        previous.filter((_, index_) => index_ !== index)
      );
  ...
  const updateEmployee = (
    ...
    setEmployees((previous) =>
      previous.map((employee, index_) =>
        index_ === index ? { ...employee, [field]: value } : employee
      )
    );
  };
  ...
  useEffect(() => {
    ...
    employees,
    openCustomPermissions.length,
    openEmployeeCombos.length,
    openRoleCombos.length,
  ]);
  ...
  const handleSubmitForm = async () => {
    ...
    try {
      ...
      for (const [index, employee] of validEmployees.entries()) {
        ...
        setSubmissionStatus(
          `Assigning employee ${index + 1} of ${totalEmployees}...`
        );
                    ...
                    <>
                      ...
                      >
                        <div className={`flex items-start gap-2`}>
                          ...
                          <p>
                            You can assign this employee to a role and customize
                            their permissions. These settings can be changed
                            later.
                          </p>
                        </div>
                      </div>
                    ...
                    >
                      <CardHeader className={`p-0`}>
                        ...
                        <p className="text-muted-foreground text-sm">
                          Override the default role permissions for this
                          specific employee.
                        </p>
                      </CardHeader>
                      <CardContent className={`p-0`}>
                        <div className="bg-warning/10 border-warning/20 rounded-lg border p-4">
                          <div className="flex items-center gap-2">
                            ...
                            <p className="text-warning text-sm">
                              Custom permissions will override the default role
            ...
            >
              <div className="flex items-start gap-2">
                ...
                <p className="text-muted-foreground text-xs">
                  Please complete all employees by selecting an employee and
