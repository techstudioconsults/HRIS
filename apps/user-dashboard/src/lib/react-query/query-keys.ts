export const queryKeys = {
  onboarding: {
    ...
    setupStatus: (employeeId: string) =>
      ['onboarding', 'setupStatus', employeeId] as const,
  },
  product: {
  ...
  file: {
    ...
    byFolder: (folderId: string, filters?: Filters) =>
      ['file', 'byFolder', folderId, filters] as const,
    // Additional file-specific query keys if needed:
    recent: (limit?: number) => ['file', 'recent', limit] as const,
    byType: (fileType: string, filters?: Filters) =>
      ['file', 'byType', fileType, filters] as const,
  },
  team: {
  ...
  payroll: {
    ...
    deductions: (filters?: Filters) =>
      ['payrolls', 'deductions', filters] as const,
    wallet: () => ['payrolls', 'wallet'] as const,
    approvedBanks: () => ['payrolls', 'approved-banks'] as const,
    payslips: (payrollId: string, filters?: Filters) =>
      ['payrolls', 'payslips', payrollId, filters] as const,
    payslipDetails: (id: string) => ['payrolls', 'payslip', id] as const,
    approvals: (payrollId: string) =>
      ['payrolls', 'approvals', payrollId] as const,
  },
  leave: {
