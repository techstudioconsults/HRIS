...
export const LeaveSetupLegacyForm = () => {
  ...
  const [leaveTypes, setLeaveTypes] = useState<LeaveSetupType[]>(
    policy?.defaultLeaveTypes?.length
      ? (policy.defaultLeaveTypes as unknown as LeaveSetupType[])
      : DEFAULT_LEAVE_TYPES
  );
  ...
  const handleAddLeaveType = () => {
    setLeaveTypes([
      ...leaveTypes,
      { name: '', days: 'Not set', cycle: 'Not set', carryOver: false },
    ]);
  };
  ...
  return (
    <section>
      ...
      <BreadCrumb
        items={[
          { label: 'Leave Hub', href: '/admin/leave' },
          { label: 'Setup' },
        ]}
        className="mb-6"
      />
                ...
                {leaveTypes.map((leaveType, index) => (
                  ...
                  >
                    <input
                      ...
                      onChange={(event) =>
                        handleLeaveTypeChange(index, 'name', event.target.value)
                      }
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        handleLeaveTypeChange(index, 'days', event.target.value)
                      }
                      className="h-12 rounded-md border px-3"
                    />
                    <input
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <label className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      {...methods.register('requireManagerApproval')}
                    />
                  ...
                  <label className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      {...methods.register('allowCarryOver')}
                    />
                    <span className="text-sm">
                      Allow unused leave days to carry over to next cycle
                    </span>
                  </label>
                </div>
