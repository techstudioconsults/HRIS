...
export class PayrollService {
  ...
  async getApprovedBanks() {
    ...
    >(`/payrolls/approved-banks`);
    if (response?.status === 200) return response.data;
  }

  async getCompanyPayrollPolicy() {
    // Ensure leading slash for consistency with other endpoints
    const response = await this.http.get<ApiResponse<CompanyPayrollPolicy>>(
      `/payroll-policy/company`
    );
  ...
  async getAllPayrolls(filters: Filters = {}) {
    const response = await this.http.get<ApiResponse<PayrollSummary>>(
      `/payrolls`,
      {
        ...filters,
      }
  ...
  async getPayrollByID(payrollId: string) {
    const response = await this.http.get<ApiResponse<Payroll>>(
      `/payrolls/${payrollId}`
    );
    if (response?.status === 200) {
      return response.data;
  ...
  async getPayrollApprovals(payrollId: string) {
    const response = await this.http.get<ApiResponse<PayrollApproval[]>>(
      `/payrolls/${payrollId}/approvals`
    );
    if (response?.status === 200) {
      return response.data;
    }
  ...
  async decidePayrollApproval(data: {
    payrollId: string;
    status: 'approved' | 'declined';
  }) {
    const response = await this.http.post<ApiResponse<PayrollApproval>>(
      `/payrolls/${data.payrollId}/approvals`,
      {
        status: data.status,
      }
    );
  ...
  async updateCompanyPayrollPolicy(data: {
    ...
    const response = await this.http.put<ApiResponse<CompanyPayrollPolicy>>(
      `/payroll-policy/company`,
      data
    );
    if (response?.status === 200) {
  ...
  async getCompanyWallet() {
    const response =
      await this.http.get<ApiResponse<CompanyWallet>>(`/wallets/company`);
    if (response?.status === 200) {
      return response.data;
    }
  }
  async updateCompanyWallet(data: {
    firstName: string;
    lastName: string;
    email: string;
    phoneNumber: string;
  }) {
    const response = await this.http.put<ApiResponse<CompanyWallet>>(
      `/wallets/company`,
      data
    );
    if (response?.status === 200) {
  ...
  async updateBonus(
    ...
    const response = await this.http.patch<ApiResponse<unknown>>(
      `/bonuses/${id}`,
      patched
    );
    if (response?.status === 200) return response.data;
