...
export const SalaryDetails = ({ payslip }: { payslip: Payslip | null }) => {
  ...
  const bonusItems = payslip
    ? mapAdjustmentsToBonusDeduction(payslip.bonuses, 'bonus')
    : [];
  const deductionItems = payslip
    ? mapAdjustmentsToBonusDeduction(payslip.deductions, 'deduction')
    : [];
  ...
  return (
    <section className="space-y-10">
      <section className="space-y-3">
        ...
        <div className="bg-primary/10 text-foreground rounded-lg px-4 py-3 text-base font-semibold">
      ...
      <section hidden={isSalaryPaid} className="space-y-4">
        <BonusDeductionManager
          type="deduction"
          profileId={profileId}
          initialItems={deductionItems}
        />
      </section>

      {/* Deductions Manager (employee-level) */}
      <section className="space-y-4">
        <Card className="bg-primary/10 rounded-2xl shadow-none">
          <CardContent className="space-y-4">
            <dl className="space-y-4 text-sm">
              <div className="flex items-center justify-between">
                ...
                <dd className="text-foreground text-base font-medium">
                  {formatCurrency(payslip?.totalBonuses || 0)}
                </dd>
              </div>
            </dl>
            <dl className="space-y-4 text-sm">
            ...
            <dl className="space-y-4 text-sm">
              <div className="flex items-center justify-between">
                <dt className="text-success text-base font-semibold">
                  Net Salary
                </dt>
                <dd className="text-success text-base font-semibold">
