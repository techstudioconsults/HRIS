...
export const SingleEmployeeForm = ({ index }: SingleEmployeeFormProperties) => {
  ...
  const { useGetTeams, useGetRoles, useGetRole, useUpdateRole } =
    useOnboardingService();
  const { data: teamsData, isLoading: loadingDepartments } = useGetTeams();
  const departmentOptions: Department[] = (teamsData || []).map(
    (team: any) => ({ id: team.id, name: team.name })
  );
  ...
  useEffect(() => {
    if (
      ...
      setCurrentPermissions({
        id: soleRole.id,
        name: soleRole.name,
        permissions: soleRole.permissions || [],
      });
    }
  ...
  return (
    <section className="w-full">
      <section className={`space-y-4`}>
        ...
        <input
          type="hidden"
          name={`employees.${index}.permissions`}
        ...
        <div>
          ...
          >
            ...
            <span>
              <Icon name="ChevronRight" size={16} className={`text-gray`} />
            </span>
          </div>
        </div>
      </section>
