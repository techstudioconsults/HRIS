...
export const AddNewMembers = ({
  ...
  const normalizedTeamId =
    typeof parentTeamId === 'string'
      ? parentTeamId
      : (parentTeamId as any)?.id || '';
  ...
  const methods = useForm({
    defaultValues: { assignments: [{ employeeId: '', roleId: '' }] },
  });

  // Derived validation
  const allValid = assignments.every(
    (a) =>
      a.employeeId.trim().length > 0 &&
      (availableRoles.length === 0 || a.roleId.trim().length > 0)
  );
  ...
  const removeRow = (index: number) => {
    ...
    setAssignments((previousAssignments) =>
      previousAssignments.filter((_, index_) => index_ !== index)
    );
    setOpenEmployeeCombos((previousCombos) =>
      previousCombos.filter((_, index_) => index_ !== index)
    );
    setOpenRoleCombos((previousRoleCombos) =>
      previousRoleCombos.filter((_, index_) => index_ !== index)
    );
  };

  const updateAssignment = (
    index: number,
    field: keyof MemberAssignment,
    value: string
  ) => {
    setAssignments((previousAssignments) =>
      previousAssignments.map((row, index_) =>
        index_ === index ? { ...row, [field]: value } : row
      )
    );
  };

  useEffect(() => {
    if (openEmployeeCombos.length !== assignments.length)
      setOpenEmployeeCombos(assignments.map(() => false));
    if (openRoleCombos.length !== assignments.length)
      setOpenRoleCombos(assignments.map(() => false));
  }, [assignments, openEmployeeCombos.length, openRoleCombos.length]);

  const toggleEmployeeCombo = (index: number) => {
    setOpenEmployeeCombos((previousCombos) =>
      previousCombos.map((openValue, index_) =>
        index_ === index ? !openValue : openValue
      )
    );
  };
  const toggleRoleCombo = (index: number) => {
    setOpenRoleCombos((previousRoleCombos) =>
      previousRoleCombos.map((openValue, index_) =>
        index_ === index ? !openValue : openValue
      )
    );
  };
  ...
  const getEmployeeLabel = (id: string) => {
    const employee = employees.find(
      (employee_: Employee) => employee_.id === id
    );
    return employee
  ...
  const handleSubmitForm = async () => {
    ...
    const valid = assignments.filter(
      (a) =>
        a.employeeId.trim() && (availableRoles.length === 0 || a.roleId.trim())
    );
    ...
    try {
      for (const [assignmentIndex, assignment] of valid.entries()) {
        ...
        if (assignmentIndex < valid.length - 1)
          ...
          <div className="bg-primary/5 border-primary/20 space-y-3 rounded-lg border p-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-muted-foreground text-sm">
                {currentIndex} of{' '}
                {assignments.filter((a) => a.employeeId.trim()).length}
              </span>
            </div>
          ...
          {[...assignments].reverse().map((assignment, reversedIndex) => {
            ...
            const isValid =
              ...
              (availableRoles.length === 0 ||
                assignment.roleId.trim().length > 0);
            return (
              <section
                key={index}
                className={cn(
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        >
                          <PopoverTrigger asChild>
                            ...
                            >
                              {assignment.employeeId
                                ? getEmployeeLabel(assignment.employeeId)
                                : 'Select member...'}
                              <Icon
                                name="ChevronsUpDown"
                                size={16}
                          ...
                          <PopoverContent
                            className="p-0 shadow-none lg:w-[588px]"
                            align="start"
                          >
                            <Command>
        ...
        {!allValid && (
          <div className="bg-warning/10 border-warning/20 rounded-lg border p-4">
            <p className="text-warning text-xs">
              Complete all member rows: select a member and role (if roles
              enabled).
            </p>
          </div>
        )}
